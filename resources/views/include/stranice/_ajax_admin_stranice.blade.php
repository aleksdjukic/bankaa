...
<div class="main-body">
    <div class="container sec-background">
        <div class="head-content">
            <p>STRANICEE</p>
            <a href="{{ route('stranice-template') }}" class="add-list tooltip" tooltip="Dodaj">
        ...
        <div class="main-body-list">
            <table>
                ...
                @foreach ($pages as $sve)
                    <tr>
                        <td>{{ $br++ }}</td>
                        <td>{{ $sve->title_sr }}</td>
                        <td><input type="text" value="{{ route('page', ["sr", $sve->slug_sr]) }}" id="myInput_{{$sve->id}}" style="width: 60%;color: white; background: #384064; border-style: none;padding: 6px;"><button onclick="copytoclip({{$sve->id}})" style="background: #384064; color: white; padding: 5px;">Copy link</button></td>
                        <td><input type="text" value="{{ route('page', ["en", $sve->slug_en]) }}" id="myInputa_{{$sve->id}}" style="width: 60%;color: white; background: #384064;border-style: none;padding: 6px;"><button onclick="copytoclipboard({{$sve->id}})"  style="background: #384064; color: white; padding: 5px;">Copy link</button></td>
                        <td>
                            <div class="edit-article tooltip" tooltip="Uredi">
                                <a href="{{ route('pages.edit', $sve->slug_sr) }}">
                            ...
                            <div class="delete-article tooltip" tooltip="Obriši">
                                ...
                                <form class='forma' method="POST" id="form_{{$sve->id}}" action="{{route('pages.destroy', $sve->slug_sr)}}">
            ...
            {{ $pages->appends(['sort'=>$sort, 'query' => $query])->links('vendor/pagination/paginate-pages-admin') }}
        </div>
    </div>
</div>
