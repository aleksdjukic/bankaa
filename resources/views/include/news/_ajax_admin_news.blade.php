...
<div class="main-body">
    <div class="container sec-background">
        <div class="head-content">
            <p>NOVOSTI</p>
            <a href="{{ route('news.create') }}" class="add-list tooltip" tooltip="Dodaj">
        ...
        <div class="main-body-list">
                        ...
                        <td>
                            ...
                            @if($sve->visibility == 1)
                                <div class="hide-article tooltip active" tooltip="Vidljivo"
                                     onclick="changeVisibility(this, {{ $sve->id }})">
                                    <img src="{{ asset('images/eye.svg') }} "
                                         alt="hide article">
                                </div>
                            @else
                                <div class="hide-article tooltip" tooltip="Sakriveno"
                                     onclick="changeVisibility(this, {{ $sve->id }})">
                            ...
                            <div class="delete-article tooltip" tooltip="Obriši">
                                <form class='forma' method="POST" id="form_{{$sve->id}}"
                                      action="{{route('news.destroy', $sve->slug_sr)}}">
            ...
            {{ $news->appends(['sort'=>$sort, 'query' => $query])->links('vendor/pagination/paginate-news-admin') }}
        </div>
    </div>
</div>
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    Swal.fire({
                        position: 'top-end',
                        icon: 'success',
                        title: 'Uspješno ste izmijenili novost!',
