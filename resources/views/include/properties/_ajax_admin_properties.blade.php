...
<div class="main-body">
    <div class="container sec-background">
        <div class="head-content">
            <p>IMOVINA</p>
            <a href="{{ route('properties.create') }}" class="add-list tooltip" tooltip="Dodaj">
        ...
        <div class="main-body-list">
                ...
                @foreach ($properties as $sve)
                    <tr>
                        ...
                        <td>
                            ...
                            @if($sve->visibility == 1)
                                <div class="hide-article tooltip active" tooltip="Vidljivo" onclick="changeVisibility(this, {{ $sve->id }})">
                                    <img src="{{ asset('images/eye.svg') }} "
                                         alt="hide article">
                                </div>
                            @else
                                <div class="hide-article tooltip" tooltip="Sakriveno" onclick="changeVisibility(this, {{ $sve->id }})">
                                    <img src="{{ asset('images/icons8_Hide_article.svg') }} "
                                         alt="hide article">
                                </div>
                            @endif
                            <div class="edit-article tooltip" tooltip="Uredi">
                                <a href="{{ route('properties.edit', $sve->slug_sr) }}">
                            ...
                            <div class="delete-article tooltip" tooltip="Obriši">
                                ...
                                <form class='forma' method="POST" id="form_{{$sve->id}}" action="{{route('properties.destroy', $sve->slug_sr)}}">
            ...
            {{ $properties->appends(['sort'=>$sort, 'query' => $query])->links('vendor/pagination/paginate-properties-admin') }}
        </div>
    </div>
</div>
@section('scripts')
    ...
    <script>
        function changeVisibility(element, id){
            // TO DO
            // showPreloader();
            ...
            if ($(element).hasClass('active')) {
                ...
                $(element).find('img').attr('src', '{{ url('/images/icons8_Hide_article.svg') }}')
            } else {
                $(element).addClass('active');
                $(element).find('img').attr('src', '{{ url('/images/eye.svg') }}')
            }

            console.log(id);
            $.ajax({
                url: '{{ route('visibility-property') }}',
                data: {'id' : id},
                success: function (response){
                    ...
                    Swal.fire({
                        position: 'top-end',
                        icon: 'success',
                        title: 'Uspješno ste izmijenili imovinu!',
